...
function KanbanBoard({ children, className }: KanbanBoardProps) {
  ...
  return (
    <SortableContext items={columnIds} strategy={rectSortingStrategy}>
      <div data-slot="kanban-board" className={cn("flex gap-3", className)}>
        {children}
      </div>
    </SortableContext>
  );
  ...
  return (
    <ColumnContext.Provider value={{ attributes, listeners, isDragging, disabled }}>
      <div
        ...
        className={cn(
          "flex flex-col rounded-xl border border-border/30 bg-background/40 backdrop-blur-sm",
...
export interface KanbanHandleProps {
  asChild?: boolean;
  className?: string;
  children?: React.ReactNode;
  cursor?: boolean;
...
function KanbanColumnHandle({ asChild, className, children, cursor = true }: KanbanHandleProps) {
  const { attributes, listeners, disabled } = React.useContext(ColumnContext);
  ...
  return (
    <Comp
      ...
      {...listeners}
      className={cn(
        cursor && !disabled && "cursor-grab active:cursor-grabbing",
...
function KanbanItemHandle({ asChild, className, children, cursor = true }: KanbanHandleProps) {
  const { listeners, disabled } = React.useContext(ItemContext);
  const Comp = asChild ? Slot : "div";
  return (
    <Comp
